...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Textarea } from "@/components/ui/textarea"
import { Plus, Edit, Trash2, ExternalLink, BookOpen, Video, FileText, Code, Star, Loader2 } from "lucide-react"
...
export default function ResourcesManagementPage() {
  ...
  useEffect(() => {
    fetchResources()
  }, [filterCategory, filterType, filterDifficulty, filterFeatured])
  ...
  const handleUpdateResource = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        setResources(prev => 
          prev.map((resource) => resource._id === editingResource._id ? data.data! : resource)
        )
        setEditingResource(null)
        resetForm()
  ...
  const toggleFeatured = async (id: string, currentlyFeatured: boolean) => {
    try {
      ...
      if (data.success && data.data) {
        setResources(prev => 
          prev.map((resource) => resource._id === id ? data.data! : resource)
        )
        toast.success(`Resource ${newFeaturedStatus ? 'featured' : 'unfeatured'} successfully!`)
      } else {
  ...
  const filteredResources = resources.filter((resource) => {
    ...
    const matchesFeatured = filterFeatured === "all" || 
      (filterFeatured === "featured" && resource.featured) ||
      (filterFeatured === "not-featured" && !resource.featured)
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-12 text-center">
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">
                {searchTerm || filterCategory !== 'all' || filterType !== 'all' || filterDifficulty !== 'all' || filterFeatured !== 'all' 
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col gap-2 ml-4">
                    <Button
                      variant={resource.featured ? "default" : "outline"}
                      size="sm"
                      onClick={() => toggleFeatured(resource._id, resource.featured)}
                      disabled={submitting}
                    >
                      <Star className="h-4 w-4 mr-2" />
                      {resource.featured ? "Featured" : "Feature"}
      ...
      {/* Edit Dialog */}
      <Dialog open={!!editingResource} onOpenChange={() => !submitting && setEditingResource(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Edit Resource</DialogTitle>
            <DialogDescription>Update resource information and details.</DialogDescription>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingResource(null)} disabled={submitting}>
              Cancel
            </Button>
