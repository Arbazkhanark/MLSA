...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { Textarea } from "@/components/ui/textarea"
import { Plus, Edit, Trash2, Heart, Calendar, MapPin, Users, Star, Loader2 } from "lucide-react"
...
export default function MomentsManagementPage() {
  ...
  const fetchMoments = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/moment?${params.toString()}`, {
        method: "GET",
        credentials: "include",
      })

      const data: ApiResponse<Moment[]> = await response.json()
  ...
  // Refetch moments when filters change
  useEffect(() => {
    fetchMoments()
  ...
  const handleAddMoment = async () => {
    try {
      ...
      const newMomentData = {
        ...
        participants: formData.participants.split(",").map((p) => p.trim()).filter(p => p.length > 0),
      ...
      const response = await fetch("/api/admin/moment", {
        method: "POST",
        headers: {
      ...
      if (data.success && data.data) {
        setMoments(prev => [data.data!, ...prev])
        setIsAddDialogOpen(false)
        resetForm()
        toast.success("Moment created successfully!")
      } else {
        toast.error(data.message || "Failed to create moment")
      }
    } catch (error) {
  ...
  const handleUpdateMoment = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/moment/${editingMoment._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.success && data.data) {
        setMoments(prev => 
          prev.map((moment) => moment._id === editingMoment._id ? data.data! : moment)
        )
        setEditingMoment(null)
        resetForm()
        toast.success("Moment updated successfully!")
      } else {
        toast.error(data.message || "Failed to update moment")
      }
    } catch (error) {
  ...
  const handleDeleteMoment = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/moment/${id}`, {
        method: "DELETE",
        credentials: "include",
      })
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Failed to delete moment")
      }
    } catch (error) {
  ...
  const toggleFeatured = async (id: string, currentlyFeatured: boolean) => {
    try {
      ...
      const response = await fetch(`/api/admin/moment/${id}/featured`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.success && data.data) {
        setMoments(prev => 
          prev.map((moment) => moment._id === id ? data.data! : moment)
        )
        toast.success(`Moment ${newFeaturedStatus ? 'featured' : 'unfeatured'} successfully!`)
      } else {
  ...
  const filteredMoments = moments.filter((moment) => {
    ...
    const matchesFeatured = filterFeatured === "all" || 
      (filterFeatured === "featured" && moment.featured) ||
      (filterFeatured === "not-featured" && !moment.featured)
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAddMoment} disabled={submitting}>
                {submitting ? (
                  <>
      ...
      {filteredMoments.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center">
            <div className="space-y-2">
              ...
              <h3 className="text-lg font-semibold">No moments found</h3>
              <p className="text-muted-foreground">
                {searchTerm || filterCategory !== 'all' || filterFeatured !== 'all' 
                  ? "No moments match your filters." 
                  : "Get started by adding your first happy moment."}
              </p>
              {!searchTerm && filterCategory === 'all' && filterFeatured === 'all' && (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMoments.map((moment) => (
            <Card key={moment._id} className="overflow-hidden">
              <div className="relative">
                <div className="h-48 bg-cover bg-center bg-gray-200" style={{ backgroundImage: `url(${moment.image})` }} />
                {moment.featured && (
                  <Badge className="absolute top-2 right-2 flex items-center gap-1">
                    <Star className="h-3 w-3" />
                    Featured
              ...
              <CardContent className="p-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    variant={moment.featured ? "default" : "outline"}
                    size="sm"
                    onClick={() => toggleFeatured(moment._id, moment.featured)}
                    disabled={submitting}
                  >
                    <Star className="h-3 w-3 mr-1" />
                    {moment.featured ? "Featured" : "Feature"}
      ...
      {/* Edit Dialog */}
      <Dialog open={!!editingMoment} onOpenChange={() => !submitting && setEditingMoment(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Edit Happy Moment</DialogTitle>
            <DialogDescription>Update moment details and information.</DialogDescription>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingMoment(null)} disabled={submitting}>
              Cancel
            </Button>
            <Button onClick={handleUpdateMoment} disabled={submitting}>
              {submitting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  Updating...
